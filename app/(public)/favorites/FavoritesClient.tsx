...
function getSortButtonClassName(isActive: boolean) {
  return isActive
    ? "h-9 rounded-full bg-[var(--desktop-nav-active)] px-3 text-[0.8rem] text-[var(--desktop-nav-active-foreground)] hover:bg-[var(--desktop-nav-active)]/95 md:h-10 md:px-4 md:text-[0.88rem]"
    : "h-9 rounded-full border-[var(--desktop-chip-border)] bg-[var(--desktop-chip)] px-3 text-[0.8rem] text-[var(--desktop-chip-foreground)] hover:border-[var(--desktop-chip-hover-border)] hover:bg-[var(--desktop-chip-hover)] hover:text-[var(--desktop-chip-hover-foreground)] md:h-10 md:px-4 md:text-[0.88rem]";
}
...
export function FavoritesClient() {
  ...
  return (
    <div className="space-y-5 pt-2 md:space-y-8 md:pt-0">
      <section className="mx-4 rounded-[1.7rem] border border-[var(--desktop-panel-border)] bg-[var(--desktop-panel)] p-4 shadow-[0_18px_38px_rgba(15,23,42,0.08)] dark:shadow-[0_18px_38px_rgba(2,6,23,0.28)] md:mx-0 md:rounded-[2rem] md:p-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between md:gap-6">
          <div className="flex items-start gap-4">
            <div className="flex size-11 items-center justify-center rounded-[1.1rem] bg-[var(--desktop-panel-soft)] text-rose-500 md:size-14 md:rounded-[1.4rem]">
              <Heart className="h-5 w-5 md:h-6 md:w-6" />
            </div>
            <div>
              <h1 className="mt-1 font-heading text-[clamp(1.9rem,7vw,3rem)] font-semibold leading-[0.96] tracking-[-0.06em] text-foreground">
                Favorites
              </h1>
              <p className="mt-2.5 max-w-2xl text-[0.84rem] leading-6 text-[var(--desktop-nav-muted)] md:mt-4 md:text-[0.98rem] md:leading-8">
        ...
        <div className="mt-4 flex flex-wrap gap-2 md:mt-6 md:gap-3">
          <span className="rounded-full bg-[var(--desktop-chip)] px-3 py-1 text-[0.74rem] font-semibold text-[var(--desktop-chip-foreground)] md:px-4 md:py-1.5 md:text-[0.82rem]">
            {songs.length} saved
          </span>
          <span className="rounded-full bg-[var(--desktop-chip)] px-3 py-1 text-[0.74rem] font-semibold text-[var(--desktop-chip-foreground)] md:px-4 md:py-1.5 md:text-[0.82rem]">
